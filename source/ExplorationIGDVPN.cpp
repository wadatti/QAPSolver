...
*******************************************************************************/
ExplorationIGDVPN::ExplorationIGDVPN(QAP* qapInst, ExpCond* cond, std::string fileName) : ExplorationBase(qapInst, cond, fileName) {
    /* 配列を確保 */
 ...
 * デストラクタ
*******************************************************************************/
ExplorationIGDVPN::~ExplorationIGDVPN() {
}

/*******************************************************************************
 * 探索の実行
*******************************************************************************/
void ExplorationIGDVPN::Explore() {
    while (status_.currTrial <= condition_->numTrial) {
        /* 初期解を生成 */
        GenerateRandSol();
        /* アルゴリズムを適用 */
        TabuSearch();
        MsgExplore("IGDVPN");  // 探索開始メッセージ
...
*******************************************************************************/
void ExplorationIGDVPN::Destruction(double ratio) {
    /* 解の破壊数を設定 */
 ...
 * タブーサーチ
*******************************************************************************/
void ExplorationIGDVPN::TabuSearch() {
    ...
    if (condition_->param.size() == 2) {
        stagnation = (int)condition_->param[0];

...
*******************************************************************************/
ExplorationNakaura::ExplorationNakaura(QAP* qapInst, ExpCond* cond, std::string fileName) : ExplorationBase(qapInst, cond, fileName) {
    /* 配列を確保 */
 ...
 * デストラクタ
*******************************************************************************/
ExplorationNakaura::~ExplorationNakaura() {
}

/*******************************************************************************
 * 探索の実行
*******************************************************************************/
void ExplorationNakaura::Explore() {
    while (status_.currTrial <= condition_->numTrial) {
        /* 初期解を生成 */
        GenerateRandSol();
        /* アルゴリズムを適用 */
        TabuSearch();
        MsgExplore("Nakaura");  // 探索開始メッセージ
 ...
 * タブーサーチ
*******************************************************************************/
void ExplorationNakaura::TabuSearch() {
    ...
    int level;        // タブー期間を管理用の仮の問題サイズ
    int subLevel;     // level管理用のカウンタ
    ...
    while (IsEndExploration() == false) {
        ...
        if (numRemnant_ > 0) {
            ...
            if (subLevel >= level) {
                ...
                aspiration = 2 * level * level;
                minTabuDur = 9 / 10 * level;
                maxTabuDur = 11 / 10 * level;
            }
            ...
            if (delta_[i][j] < 0) {
                ...
                if (r == NA) {
                    ...
                    if (status_.currCost < qapInstance_->cost_) {
                        qapInstance_->cost_ = status_.currCost;
                        for (i = 0; i < qapInstance_->size_; ++i) {
                            qapInstance_->pi_[i] = p_[i];
                        }
                    }

                    /* 差分リストを更新 */
                    for (i = 0; i < qapInstance_->size_ - 1; ++i) {
                        for (j = i + 1; j < qapInstance_->size_; ++j) {
                            if (delta_[i][j] != INF) {
                                if (i != r && i != s && j != r && j != s) {
